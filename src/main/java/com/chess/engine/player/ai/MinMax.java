...
public final class MinMax implements MoveStrategy {
    ...
    public MinMax(final int searchDifficulty) {
        ...
        for (final Move move : board.currentPlayer().getLegalMoves()) {
            ...
            if (moveTransition.getMoveStatus().isDone()) {
                ...
                currentValue = board.currentPlayer().getAlliance().isWhite() ?
                                min(moveTransition.getToBoard(), this.searchDifficulty - 1) :
                                max(moveTransition.getToBoard(), this.searchDifficulty - 1);
    ...
    private int min(final Board board,
        ...
        for (final Move move : board.currentPlayer().getLegalMoves()) {
            final MoveTransition moveTransition = board.currentPlayer().makeMove(move);
            if (moveTransition.getMoveStatus().isDone()) {
                final int currentValue = max(moveTransition.getToBoard(), difficulty - 1);
    ...
    private int max(final Board board,
        ...
        for (final Move move : board.currentPlayer().getLegalMoves()) {
            final MoveTransition moveTransition = board.currentPlayer().makeMove(move);
            if (moveTransition.getMoveStatus().isDone()) {
                final int currentValue = min(moveTransition.getToBoard(), difficulty - 1);
